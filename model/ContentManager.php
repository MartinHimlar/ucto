...
class ContentManager extends \Nette\Object {
    ...
    const
		...
		COLUMN_ZOBRAZ = 'zobraz',
                COLUMN_LINK = 'link',
                COLUMN_DEFAULT = 'vychozi';
    ...
    public function findEnabled($enabled){
        if($enabled)
            ...
            $zobraz = 0;
        return $this->findAll()->where(self::COLUMN_ZOBRAZ, $zobraz);
    }
    ...
    public function nullDefault(){
        ...
        foreach($rows as $row){
            if($row->vychozi != 0){
                $this->update($row->id,$row->title,$row->content,$row->zobraz,$row->link,0);
            }
        }
    }
}
