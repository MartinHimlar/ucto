...
class UzivatelePresenter extends BasePresenter
{
	...
	public function startup()
	{
		...
		if (!$this->user->isLoggedIn() || $this->user->isInRole(self::ROLE_OWNER)) {
	...
	{
		if (!$this->user->isInRole(self::ROLE_OWNER) && $this->user->isLoggedIn()) {
			$this->template->uzivatele = $this->users->findAll();
			$this->template->activeUser = $this->getUser()->id;
			$this->template->opravneni = $this->user->isInRole(self::ROLE_ADMINISTRATOR);
		} else {
			$this->flashMessage("Litujeme, nemáte dostatečné oprávnění.");
			$this->redirect('Homepage:');
		}
	...
	{
		if (!$this->user->isInRole(self::ROLE_ADMINISTRATOR)) {
			$this->flashMessage("Litujeme, nemáte dostatečné oprávnění.");
			$this->redirect('Homepage:');
		}
	}
	...
	{
		if (!$this->user->isInRole(self::ROLE_OWNER)) {
			if ($id != null) {
	...
	protected function createComponentNovyUzivatel()
	{
		...
		$opravneni = array(
			self::ROLE_REGISTERED => 'Registrovaný',
			self::ROLE_OWNER => 'Kontrolor',
			self::ROLE_ADMINISTRATOR => 'administratoristrator'
		);
		...
		$form->addText('username', 'Uživatelské jméno:')
			->setDisabled(!$this->user->isInRole(self::ROLE_ADMINISTRATOR))
